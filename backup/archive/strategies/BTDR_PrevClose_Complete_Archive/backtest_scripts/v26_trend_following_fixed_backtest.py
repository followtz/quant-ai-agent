# -*- coding: utf-8 -*-
"""
V2.6 纯趋势跟踪版 - 修复胜率统计Bug

核心逻辑：
1. 突破前N日高点 → 做多（加仓）
2. 跌破前N日低点 → 平仓（减仓）
3. 移动止盈：从最高价回撤X% → 平仓
4. 底仓=1000股，最大持仓11000股，最小持仓7000股

修复内容：
1. 所有交易都计入total_trades/wins/losses
2. 修复移动止盈的盈亏计算（用entry价格，不是peak_price）
3. 增加交易类型统计（突破买入/破位卖出/移动止盈）
"""
import sys, json
...
# ===================== V2.6 纯趋势跟踪回测（修复版） =====================
def backtest_v26_trend_following_fixed(data, params=None):
    """
    V2.6 回测引擎 - 纯趋势跟踪（修复版）
...
# ===================== 参数扫描 =====================
def param_sweep_v26_fixed(data):
    """扫描V2.6的关键参数（修复版）"""
    results = []
    ...
    for breakout_days in breakout_days_range:
        for stop_loss in stop_loss_range:
            for position_size in position_size_range:
                ...
                result = backtest_v26_trend_following_fixed(data, params)
...
def main():
    ...
    base_result = backtest_v26_trend_following_fixed(data, base_params)
    ...
    results = param_sweep_v26_fixed(data)
    
    print(f"  扫描完成，共{len(results)}个组合")
    print(f"\n  Top 10 (按盈亏排序):")
    print(f"  {'突破N日':<8} {'止盈':<8} {'加减仓':<8} {'盈亏':<12} {'总交易':<8} {'胜率':<8} {'回撤':<10}")
    print("  " + "-"*70)
    ...
    print(f"  V2.6基准(未修复):             $1,700 (胜率0.0%, 回撤-159.59%)")
    print(f"  V2.6修复基准(底仓1000股):      ${base_result['total_pnl']:,.2f} "
          f"(胜率{base_result['win_rate']:.1f}%, 回撤{base_result['max_drawdown']:.2f}%)")
    print(f"  V2.6修复最优(底仓1000股):      ${best['total_pnl']:,.2f} "
    ...
    if best['total_pnl'] > 2115 and best['max_drawdown'] > -30:
        print(f"  [部分成功] 盈亏(${best['total_pnl']:,.2f})超过V2，但回撤({best['max_drawdown']:.2f}%)仍高")
        print(f"  建议：降低仓位或换低波动标的")
    elif best['max_drawdown'] < -30:
        print(f"  [失败] 回撤({best['max_drawdown']:.2f}%)仍然过大")
        print(f"  根本原因：BTDR波动率93.4%，不适合同趋势跟踪策略")
        print(f"  建议：更换低波动(<30%)标的，或接受V2原版的结果")
    else:
        print(f"  [成功] V2.6修复版(${best['total_pnl']:,.2f})综合表现最好")
        print(f"  盈亏=${best['total_pnl']:,.2f}, 回撤={best['max_drawdown']:.2f}%, 胜率={best['win_rate']:.1f}%")
    
    # 5. 保存报告
    report = {
        'timestamp': datetime.now().isoformat(),
        'v26_base_fixed': base_result,
        'v26_best_fixed': best,
        ...
        'final_conclusion': {
            'best_version': 'v26_best_fixed' if best['total_pnl'] > 2115 else 'v2_original',
            'reason': 'V2.6修复版盈亏更高但回撤大' if best['total_pnl'] > 2115 else 'V2原版回撤相对可控',
        }
    }
    
    out_path = Path(WORKSPACE) / "data" / "history" / "v26_trend_following_fixed_report.json"
